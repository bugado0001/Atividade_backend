/*7. Faça um script que leia duas notas de um aluno, calcule e escreva a média final deste aluno. Considerar que a média é ponderada e que o peso das notas é 4 e 6. */

const prompt = require('prompt-sync')();

let notaUm = Number(prompt(console.log("Insira a primeira nota: ")))
let notaDois = Number(prompt(console.log("Insira a primeira segunda: ")))

let mediaFinal = (notaUm * 4 + notaDois * 6) / 10

console.log(`Media final: ${mediaFinal}`)
